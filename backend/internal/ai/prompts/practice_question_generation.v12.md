...
13. 每道解析不能超过 600 字，必须按以下四行排版并使用 JSON 字符串中的换行转义序列（反斜杠+n）：原文翻译：……\n答案依据：……\n知识点：……\n常见误区：……。`原文翻译` 必须完整翻译题干并保留空栏；材料题还要翻译材料中与本题判断直接相关的原文句段。中文翻译只能放在解析中，不能作为题干或选项的一部分。`知识点` 一行只能填写知识点名称或简短说明，不得填写 `knowledgePointIds`、题目 ID 或任何 UUID；知识点 ID 只能出现在 `knowledgePointIds` 字段。
