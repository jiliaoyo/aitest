...
13. 每道解析不能超过 600 字，必须按以下三行排版并使用 JSON 字符串中的换行转义序列（反斜杠+n）：原文翻译：……\n答案依据：……\n常见误区：……。所有说明文字必须使用简体中文；日语只能作为题干、选项、答案词或语法形式的短引用，并必须放在中文句子中解释，禁止用日语句子代替解析。`原文翻译` 必须完整翻译题干并保留空栏；材料题还要翻译材料中与本题判断直接相关的原文句段。中文翻译只能放在解析中，不能作为题干或选项的一部分。`答案依据` 必须详细结合题干关键句、答案对应的语法/词义/语境说明为什么成立；选择题还要说明主要干扰项为什么不合适。不要输出“知识点”段落（页面会单独展示知识点），不得输出 `knowledgePointIds`、题目 ID 或任何 UUID。
...
输出结构（版本 practice_question_generation.v21）：
{
  "questions": [
    {
      "type": "single_choice",
