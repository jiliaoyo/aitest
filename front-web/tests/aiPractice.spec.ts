...
describe('AI 个性化练习', () => {
  it('把当前级别和选定题型交给 AI 生成队列并进入生成批次', async () => {
    ...
    expect(wrapper.get('#ai-category').text()).toContain('愿望、计划与推量')
    expect(wrapper.get('#ai-category').text()).not.toContain('条件、假定与逆接')
    expect(wrapper.get('#ai-category').text()).not.toContain('可能、被动、使役')
